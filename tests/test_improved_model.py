...
def test_multi_start_optimization():
    """Test that multi-start finds better solutions."""
    cost_matrix, _ = calculate_cost_matrix('data/synthetic/synthetic_network.csv')
    cost_matrix_normalized = (cost_matrix - np.min(cost_matrix)) / (np.max(cost_matrix) - np.min(cost_matrix) + 1e-6)
    ...
    # Single restart
    path_single = model.predict(source, destination, num_restarts=1, validate=False)
    cost_single = model._calculate_path_cost(path_single) if path_single else float('inf')
    
    # Multiple restarts
    path_multi = model.predict(source, destination, num_restarts=3, validate=False)
    cost_multi = model._calculate_path_cost(path_multi) if path_multi else float('inf')
    
    # Multi-start should find equal or better solution
    assert cost_multi <= cost_single * 1.1  # Allow 10% tolerance
...
def test_early_stopping():
    ...
    # If it takes less than expected, early stopping worked
    assert elapsed < 10.0  # Should be much faster
