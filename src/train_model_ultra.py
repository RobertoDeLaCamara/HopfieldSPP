...
class UltraHopfieldLayer(Layer):
    ...
    # distinct check for GPU availability for XLA
    _gpu_available = len(tf.config.get_visible_devices('GPU')) > 0

    @tf.function(jit_compile=_gpu_available)  # XLA compilation only if GPU is available
